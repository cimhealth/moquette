...
public class RedisMessagesStore extends MapDBMessagesStore implements IMessagesStore {
    ...
    private RedisTemplate<String, StoredMessage> m_persistentMessageStore;
...
//        this.stringRedisTemplate = new StringRedisTemplate(connectionFactory);
        this.m_persistentMessageStore = new RedisTemplate<String, StoredMessage>();
        this.m_persistentMessageStore.setConnectionFactory(connectionFactory);
        this.m_persistentMessageStore.setKeySerializer(new StringRedisSerializer());
//        this.m_persistentMessageStore.setValueSerializer(new Jackson2JsonRedisSerializer(StoredMessage.class));
        this.m_persistentMessageStore.setValueSerializer(new JdkSerializationRedisSerializer());
        this.m_persistentMessageStore.afterPropertiesSet();
//        this.m_retainedStore = new StringRedisTemplate();
...
//        m_retainedStore.opsForValue().set(topic, guid);
    ...
    public String storePublishForFuture(StoredMessage evt) {
        ...
        evt.setGuid(guid);
        m_persistentMessageStore.opsForValue().set(guid, evt);
//        m_persistentMessageStore.opsForHash().put(evt.getClientID(), evt.getMessageID(), guid);
    ...
    public List<StoredMessage> listMessagesInSession(Collection<String> guids) {
        ...
        for (String guid : guids) {
            ret.add(m_persistentMessageStore.opsForValue().get(guid));
        }
        return ret;
    }
...
//        m_persistentMessageStore.opsForHash().delete(clientID);
//        m_persistentMessageStore.remove(clientID); //TODO？？
    }

    @Override
    public StoredMessage getMessageByGuid(String guid) {
        return m_persistentMessageStore.opsForValue().get(guid);
    }
